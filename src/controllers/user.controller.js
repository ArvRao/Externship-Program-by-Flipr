...
const getUsers = async (req, res) => {
  try {
    ...
    // fetch all users sorted by username if query is not provided
  ...
  try {
    ...
    if (existingUser)
      return res.status(400).json({
        success: false,
        message: "User already exists with that username. Please login or choose a different username",
      });
    ...
    if (existingVerifiedUser)
      return res.status(400).json({
        success: false,
        message: "User already exists with that email. Please login or choose a different email",
      });
    ...
    res.status(201).json({
      success: true,
      message: "New user created and verification mail is sent. Please verify your email to login",
      user: newUser,
    });
  } catch (error) {
    res
  ...
  try {
    ...
    if (username) {
      ...
      if (existingUser)
        return res.status(400).json({
          success: false,
          message: "User already exists with that username. Please choose a different username",
        });
...
const verifyEmail = async (req, res) => {
  try {
    ...
    await User.findByIdAndUpdate(decodedData.id, {
      email_verified: true
    });
...
const sendFriendRequest = async (req, res) => {
  try {
    ...
    // Validation: check if that user is a friend already
    ...
    // Validation: check if the user is not blocked
    if (
      ...
      return res.status(400).json({
        success: false,
        message: "This is a blocked user. So cannot send friend request",
      });

    // Validation: check if the friendId is already present in pending requests
    if (user.friendRequests.includes(mongoose.Types.ObjectId(friendId)))
      return res
        .status(400)
        .json({
          success: false,
          message: "Friend request is already present"
        });
...
// accept a friend request sent by other user
const acceptFriendRequest = async (req, res) => {
  try {
    ...
    // Validation: check if friendId is present in pending requests
    if (!user.friendRequests.includes(mongoose.Types.ObjectId(friendId)))
      return res.status(404).json({
        success: false,
...
const blockUser = async (req, res) => {
  try {
    ...
    // Validation: check if blockUserId is valid
